...
class SiteController extends Controller
{
    ...
    public function actionLink() {
        ...
        $queryUser = Userlog::find()
            ->joinWith(['host' => function ($q) {$q->select(['id_host', 'host']);}])
//            ->joinWith(['users' => function($q) { $q->select(['id_user','login', 'fio', 'post', 'depart']); }])
            ->joinWith(['users' ])
            ->joinWith(['ip' => function($q) { $q->select(['id_ip','client_ip']); }])
...
//            ->andFilterWhere(['not like', 'users.login', ['17']])
    ...
    public function actionAdd()
    {
        ...
        if(isset($user)){
            ...
            $model->id_osversion = OsVersion::getId($osVersion);
            $model->id_processor = Processor::getId($Processor);
